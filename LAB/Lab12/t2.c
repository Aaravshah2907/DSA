...
void bfs(Graph *G, int s){
    ...
    while (!is_empty(Q)){
        ...
        // For each white node v adjacent to u:
        for (int v = 0; v < G->V; v++){
            if (G->adjacency_matrix[u][v] == 1 && G->vertices[v].c == WHITE){ /* v is adjacent to u and is white */
                // Mark v as grey
                G->vertices[v].c = GREY;
...
int main(int argc, char *argv[]){
    ...
    if (argc < 2){
        printf("Usage: %s <input_file> <vertex number>\n", argv[0]);
        return 1;
    }
    ...
    // Convert source vertex from string to int
    int s = atoi(argv[2]);
